...
function LoadMenuItm(menuItems){
...
function loadMenuButton(menu){ 
     let menuButton = menu.map(item=>{return item.category});
     menuButton.push('all')
     menuButton = menuButton.filter((c,index)=>{
      return menuButton.indexOf(c) == index;  
     }).toSorted();
    
    let button = menuButton.map(btn=>{
    ...
    filterBtns.forEach(btn=>{ 
      btn.addEventListener('click',(e)=>{
         ...
         const result = menu.filter((x)=>{return (x.category===catagory)});
         if (catagory === "all") {
          LoadMenuItm(menu);
        } else {
          LoadMenuItm(result);
